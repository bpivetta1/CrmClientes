  ...
  return new Promise(async (resolve, reject) => {
    try {
      (async () => {
        ...
        // 🚫 Remover makeInMemoryStore — Baileys 7 não usa mais isso
        // const store = makeInMemoryStore({ logger: loggerBaileys });

        // ✅ Estado de autenticação com cache otimizado
        ...
        // 🚫 Não existe mais "store" para vincular
        // wsocket.store = store;
        // store.bind(wsocket.ev);

        // ✅ Restante do seu código continua igual
        // (connection.update, QRCode, reconnect, etc.)
